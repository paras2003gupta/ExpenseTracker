...
fun AddTransactionSheet(
    groups: List<com.expenseit.core.model.GroupEntity> = emptyList(),
    onDismiss: () -> Unit,
    onSave: (merchant: String, description: String, amount: Double, category: String, dateMillis: Long, splitGroupId: String?) -> Unit,
    ...
    var selectedDateMillis by remember { mutableStateOf(DateUtils.now()) }
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                placeholder = { Text("e.g. Dinner order") },
                modifier = Modifier.fillMaxWidth(),
                singleLine = true
            )
            ...
            Row(
                ...
                // Date Picker Input
                OutlinedTextField(
                    value = DateUtils.formatDate(selectedDateMillis),
                    onValueChange = {},
                    label = { Text("Date") },
                    readOnly = true,
                    trailingIcon = {
            ...
            // Split with Group selection dropdown
            Box(modifier = Modifier.fillMaxWidth()) {
                val groupText = if (selectedGroupId == null) {
                    "Personal Expense (No Split)"
                } else {
                    groups.find { it.id == selectedGroupId }?.name ?: "Personal Expense (No Split)"
                }

                OutlinedTextField(
                    value = groupText,
                    onValueChange = {},
                    label = { Text("Split with Group") },
                    readOnly = true,
                    trailingIcon = {
                ...
                Button(
                    onClick = {
                        ...
                        if (merchant.trim().isEmpty()) {
                            ...
                            errorMsg = ""
                            onSave(merchant, description, amountVal, selectedCategory, selectedDateMillis, selectedGroupId)
                        }
                    }
                ) {
    ...
    if (showDatePicker) {
        DatePickerDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        selectedDateMillis = datePickerState.selectedDateMillis ?: DateUtils.now()
